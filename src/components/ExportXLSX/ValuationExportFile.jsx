...
const ValuationExportFile = ({ data }) => {
    const flattenData = (data) => {
        return data.map((item) => {
          const flattenedItem = {
            'Supervisor': item?.Supervisor?.FirstName[0] === undefined ? '' : item?.Supervisor?.FirstName[0] + '.' + ' ' + item?.Supervisor?.LastName,
            'Surname and name': item.Employee.FullName,
          ...
          flattenedItem['%Production \n serial+aftermarket'] = Math.round(item?.Valuation * 1000) / 10 + "%";
      ...
      const generateExcel = async () => {
        ...
        const worksheet = workbook.addWorksheet('valuation');
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            row.eachCell((cell, cellNumber) => {
                ...
                if(columnName !== 'Surname and name' && columnName !== 'Position' && columnName !== 'Supervisor')
                {
        ...
        downloadLink.download = 'valuation.xlsx';
