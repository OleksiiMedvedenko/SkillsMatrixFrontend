...
const Department = () => {
  ...
  const { data: auditsInfo, loading, error } = useFetch(REACT_APP_API_URL + "audit/GetActualAreaAuditsLevel/" + area?.AreaId);
            ...
            <Box display="flex" justifyContent="space-between" flex="1 1 20%" backgroundColor={colors.primary[400]} p="15px" borderRadius="4px" alignItems="center">
                <Box display="flex" alignItems="center">
                    <Box display="flex" justifyContent="space-between">
                        <Box display="flex" justifyContent="space-between">
                            <Typography style={{color: colors.greenAccent[500] }} variant="h4">Audit dates for the department { departments?.find(d => d?.DepartmentId === department)?.Name }, area: </Typography>
                        </Box>
                    </Box>
